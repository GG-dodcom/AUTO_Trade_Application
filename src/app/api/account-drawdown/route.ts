// src/app/api/account-drawdown/route.ts
...
export async function GET(request: Request) {
	...
	try {
		let query = `
      SELECT entry_time, drawdown
      FROM trades
      WHERE accountId = ? AND status = 'Closed' AND drawdown IS NOT NULL
    `;
		...
		const drawdowns = await db.all(query, params);

		if (!drawdowns || drawdowns.length === 0) {
			return NextResponse.json(
				{ error: "No closed trades with drawdown data found for this account" },
				{ status: 404 }
			);
		}

		return NextResponse.json(drawdowns, { status: 200 });
	} catch (error: any) {
		console.error(
			"Error fetching account drawdown:",
			error.message,
			error.stack
		);
		return NextResponse.json(
